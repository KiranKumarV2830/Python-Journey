# 🐍 Python Journey

# 🚀 From Beginner to AI Engineer

Learning Python one day at a time.

---

# 🏷️ Badges

![Python](https://img.shields.io/badge/Python-3.14-blue?style=for-the-badge&logo=python)
![Learning](https://img.shields.io/badge/Learning-Daily-success?style=for-the-badge)
![Days](https://img.shields.io/badge/Day-24-orange?style=for-the-badge)
![Projects](https://img.shields.io/badge/Mini%20Projects-11-blueviolet?style=for-the-badge)

![Last Commit](https://img.shields.io/github/last-commit/KiranKumarV2830/Python-Journey?style=for-the-badge)
![Repo Size](https://img.shields.io/github/repo-size/KiranKumarV2830/Python-Journey?style=for-the-badge)
![Stars](https://img.shields.io/github/stars/KiranKumarV2830/Python-Journey?style=for-the-badge)
![Forks](https://img.shields.io/github/forks/KiranKumarV2830/Python-Journey?style=for-the-badge)

---

# 📖 About

Welcome to my **Python Journey** repository.

This repository documents my day-by-day Python learning journey from beginner to advanced.

Everything I learn is uploaded here with:

- 📚 Notes
- 💻 Coding Problems
- 📝 Practice Sheets
- 🐞 Debugging Challenges
- 🎤 Interview Questions
- 🚀 Mini Projects
- 📄 README Files

My goal is to build strong programming fundamentals and eventually become an **AI Engineer**.

---

# 🎯 Goals

- ✅ Master Python Fundamentals
- ⬜ Improve Problem Solving
- ⬜ Learn Data Structures & Algorithms
- ⬜ Learn Object-Oriented Programming
- ⬜ Build Real-World Projects
- ⬜ Learn Machine Learning
- ⬜ Learn Artificial Intelligence
- ⬜ Become an AI Engineer

---

# 📚 Learning Progress

| Day    | Topic                          | Status |
| ------ | ------------------------------- | ------ |
| Day 01 | Python Basics                   | ✅      |
| Day 02 | Variables & Data Types          | ✅      |
| Day 03 | Strings                         | ✅      |
| Day 04 | User Input & Type Casting       | ✅      |
| Day 05 | Operators                       | ✅      |
| Day 06 | Comparison Operators            | ✅      |
| Day 07 | Nested If & Logical Operators   | ✅      |
| Day 08 | Match Case                      | ✅      |
| Day 09 | For Loops                       | ✅      |
| Day 10 | While Loops                     | ✅      |
| Day 11 | Functions                       | ✅      |
| Day 12 | Parameters & Arguments          | ✅      |
| Day 13 | Return Statement                | ✅      |
| Day 14 | Variable Scope                  | ✅      |
| Day 15 | Lists                           | ✅      |
| Day 16 | List Methods                    | ⬜      |
| Day 17 | 2D Lists                        | ⬜      |
| Day 18 | Tuples                          | ✅      |
| Day 19 | Sets                            | ✅      |
| Day 20 | Dictionaries                    | ✅      |
| Day 21 | Loops with Collections          | ✅      |
| Day 22 | Exception Handling              | ✅      |
| Day 23 | File Handling                   | ✅      |
| Day 24 | Modules & Imports               | ✅      |
| Day 25 | Virtual Environments & pip      | ⬜      |
| Day 26 | Python Libraries & Packages     | ⬜      |
| Day 27 | Comprehensions                  | ⬜      |
| Day 28 | `*args` & `**kwargs`            | ⬜      |
| Day 29 | Lambda Functions                | ⬜      |
| Day 30 | OOP - Classes & Objects         | ⬜      |
| Day 31 | Constructors & `__init__()`     | ⬜      |
| Day 32 | Instance & Class Variables      | ⬜      |
| Day 33 | Encapsulation                   | ⬜      |
| Day 34 | Inheritance                     | ⬜      |
| Day 35 | Polymorphism                    | ⬜      |
| Day 36 | Abstraction                     | ⬜      |
| Day 37 | Advanced OOP                    | ⬜      |
| Day 38 | Decorators                      | ⬜      |
| Day 39 | Iterators                       | ⬜      |
| Day 40 | Generators                      | ⬜      |

---

# 📂 Repository Structure

```
Python-Journey/
│
├── Day-01/
├── Day-02/
├── Day-03/
├── Day-04/
├── Day-05/
├── Day-06/
├── Day-07/
├── Day-08/
├── Day-09/
├── Day-10/
├── Day-11/
├── Day-12/
├── Day-13/
├── Day-14/
├── Day-15/
├── Day-16/
├── Day-17/
├── Day-18/
├── Day-19/
├── Day-20/
├── Day-21/
├── Day-22/
├── Day-23/
├── Day-24/
│
├── Mini-Projects/
│   ├── ATM PIN Verification/
│   ├── Calculator/
│   ├── Discount Calculator/
│   ├── Madlibs Game/
│   ├── Movie Ticket Booking/
│   ├── Student Result Calculator/
│   ├── Student Marks Analyzer/
│   ├── Shopping Cart/
│   ├── Weight Converter/
│   ├── Temperature Converter/
│   └── Notes App CLI/
│
├── Practice-Sheets/
│
├── Resources/
│
├── README.md
│
└── LICENSE
```

---

# 🧠 How I'm Learning

Each day follows the same structure:

1. **Concept** — definitions and code examples for every subtopic
2. **Practice Questions** — hands-on problems to lock in the concept
3. **Interview Questions** — theory questions asked out loud, like a real interview
4. **Mini Project** — a small working program combining everything from that day

This keeps learning consistent, practical, and portfolio-ready from Day 1.

---

# 🙋 Author

**Kiran Kumar V**
📍 Bangalore, India
🎯 Goal: Generative AI Engineer
GitHub: [@KiranKumarV2830](https://github.com/KiranKumarV2830)